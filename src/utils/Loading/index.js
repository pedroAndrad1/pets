...
import { Wrapper, StyledLoading } from './styles';

/**Cada grupo (g) e um frame da animation e cada frame tem 300ms na tela.
 * Existem quatro grupos, 0, 1, 2 e 3.
 */

const Loading = () => {

    /**O frame inicial sera o 0 */
    ...
    useEffect( () => {  
        ...
        const updateStep = () => {
            setStep( step => {
                //Se ainda nao tiver chegado a terceira step
                if(step < 3) return step + 1;
                //Se ja tiver chegado animation deve ser reiniciada
                else return 0;
            })
        }

        //Chamando updateStep a cada 300ms com um setInterval.
        //Estou salvando numa const para limpar o intervalo no dismount deste component.
        //Caso contrario haver vazamento de memoria.
        const interval = setInterval(updateStep, 300);

        //Isso e chamado no dismount do component
        return () => {
    ...
    //Define qual frame sera mostrado. Se o numero do frame for igual a step, ele sera mostrado.
    const displayStep = frame => {
        return {
          display: step === frame ? 'block' : 'none',
        };
      }
